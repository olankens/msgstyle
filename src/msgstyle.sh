...
assert_no_scope_or_type() {
    ...
    if [[ "$payload" != *"["* && "$payload" != *"]"* ]]; then return 0; fi
...
assert_no_trailing_punctuation() {
    ...
    if [[ "$payload" != *"."* && "$payload" != *","* && "$payload" != *":"* ]]; then return 0; fi
...
assert_start_with_allowed_verb() {
    ...
    for element in "${allowed[@]}"; do [[ "$payload" == "$element" ]] && exit 0; done

    echo "❌ Commit message must start with an allowed verb: ${allowed[*]}"
    echo "Your message starts with: '$payload'"
    exit 1
