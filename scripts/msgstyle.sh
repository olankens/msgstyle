...
assert_start_with_verb() {
  ...
  local element=$(head -n1 "$1" | awk '{print tolower($1)}' | tr -d '[:punct:]' | tr -d '\r')
  # curl "https://api.dictionaryapi.dev/api/v2/entries/en/$element" | jq -e 'type=="array" and any(.[].meanings[]?; .partOfSpeech=="verb")' >/dev/null && return 0
  curl -sf "https://api.datamuse.com/words?sp=$element&md=p" | jq -e 'any(.[]; .word=="'"$element"'" and any(.tags[]?; .=="v"))' >/dev/null && return 0
...
main() {
  ...
  assert_length_between_20_and_100 "$1"
